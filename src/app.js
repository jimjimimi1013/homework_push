...
function App() {
    ...
    const saveAssign = async (x) => { if (!user)
        ...
        if (!active)
            void sendPush('assignment', { title: `[${x.title}]`, body: '새 숙제가 등록됐어요.', url: makeDeepLink('assignment', { assignmentId: next[next.length - 1]?.id }), eventId: `assignment-${next[next.length - 1]?.id}` });
        setActive(null);
        backPage('teacher');
