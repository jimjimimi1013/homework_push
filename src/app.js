    ...
    return React.createElement(Frame, null,
        ...
        React.createElement("div", { className: "flex-1 min-h-0 overflow-y-auto px-4 py-4" },
            ...
            tab === 'contact' && React.createElement(React.Fragment, null,
                React.createElement("h1", { className: "text-[26px] leading-9 font-black" }, "일하기 싫어서 만들어봤어요 🤦"),
                React.createElement("p", { className: "mt-1 mb-4 text-center text-[12px] leading-[19px] text-[#777]" }, "딴짓할 때가 제일 재밌어요. 여러분도 심심할 때 아무거나 보내보세요! 🤭"),
                React.createElement("div", { className: "overflow-hidden rounded-2xl border bg-white mb-6", style: { borderColor: LIST_BORDER } }, CONTACT_MESSAGES.map((message, index) => React.createElement("button", { key: message, onClick: () => chooseContact(message), className: `w-full min-h-[50px] px-3 py-2 text-left flex items-center gap-2 active:bg-[#FFF8F7] ${index < CONTACT_MESSAGES.length - 1 ? 'border-b border-[#EEEEEE]' : ''}` },
