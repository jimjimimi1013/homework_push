    ...
    return React.createElement(Frame, null,
        ...
        tab !== 'contact' && React.createElement("div", { className: "flex-1 min-h-0 overflow-y-auto px-4 py-4" },
            ...
            tab === 'notifications' && React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "space-y-2" }, myNotices.length ? myNotices.map(n => { const assignment = assigns.find(a => a.id === n.assignmentId); const noticeColor = assignment ? assignmentTypeColor(assignment.type) : LIST_BORDER; return React.createElement("div", { key: n.id, className: "bg-white rounded-2xl border p-4", style: { borderColor: noticeColor } },
                    React.createElement("div", { className: "flex items-start gap-3" },
                        n.kind === 'contact' ? React.createElement("div", { className: "min-w-0 flex-1 text-left", style: { color: noticeColor } },
                            React.createElement("span", { className: "block text-[13px] font-medium" }, n.sender),
                            React.createElement("p", { className: "mt-1 text-[13px] font-medium leading-relaxed whitespace-pre-wrap" }, n.contactMessage)) : React.createElement("button", { onClick: () => onOpenNotice(n), className: "min-w-0 flex-1 text-left text-[13px] font-medium leading-relaxed whitespace-pre-wrap", style: { color: noticeColor } }, n.message),
        ...
        tab === 'contact' && React.createElement("div", { className: "flex-1 min-h-0 overflow-y-auto px-4 py-4", style: { WebkitOverflowScrolling: 'touch', overflowX: 'hidden', touchAction: 'pan-y' } },
                React.createElement("div", { className: "mb-4" },
                    ...
                    React.createElement("p", { className: "mt-1 text-center text-[13px] leading-[20px] text-[#777]" },
                        "선생님과 학생들에게 메세지를 보낼수 있습니다.", React.createElement("br"),
                        "아래 리스트에서만 가능합니다.", React.createElement("br"),
                        "심심할때 한번씩 보내보세요!")),
                React.createElement("div", { className: "rounded-2xl border bg-white mb-6", style: { borderColor: LIST_BORDER } }, CONTACT_MESSAGES.map((message, index) => React.createElement("button", { key: message, onClick: () => chooseContact(message), className: `w-full min-h-[50px] px-3 py-2 text-left flex items-center gap-2 active:bg-[#FFF8F7] ${index < CONTACT_MESSAGES.length - 1 ? 'border-b border-[#EEEEEE]' : ''}` },
    ...
    return React.createElement(Frame, null,
        ...
        React.createElement("div", { className: "flex-1 overflow-y-auto p-4" },
            ...
            tab === 'notifications' && React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "space-y-2" }, teacherNotices.length ? teacherNotices.map(n => { const assignment = assigns.find(a => a.id === n.assignmentId); const noticeColor = assignment ? assignmentTypeColor(assignment.type) : LIST_BORDER; return React.createElement("div", { key: n.id, className: "bg-white rounded-2xl border p-4", style: { borderColor: noticeColor } },
                    React.createElement("div", { className: "flex items-start gap-3" },
                        React.createElement("button", { onClick: () => onOpenNotice(n), className: "min-w-0 flex-1 text-left text-[13px] font-medium leading-relaxed whitespace-pre-wrap", style: { color: noticeColor } }, n.message),
                        React.createElement("button", { onClick: () => onDismissNotice(n.id), className: "shrink-0 text-[12px] font-bold text-[#999]", "aria-label": "알림 삭제" }, "삭제")),
                    React.createElement("div", { className: "mt-2 text-[10px] text-[#AAA]" }, n.createdAt)); }) : React.createElement(Empty, null, "새 알림이 없어요."))),
