...
function UploadBox({ fileName, onChange }) {
    ...
    if (androidPwa)
        return React.createElement("button", { type: "button", onClick: pickAndroidFile, className: "w-full h-[122px] rounded-[16px] border-2 border-dashed bg-white cursor-pointer flex flex-col items-center justify-center", style: { borderColor: '#29ADBD' } }, boxContent);
    return React.createElement("label", { className: "block w-full h-[122px] rounded-[16px] border-2 border-dashed bg-white cursor-pointer flex flex-col items-center justify-center", style: { borderColor: '#29ADBD' } },
        React.createElement("input", { type: "file", accept: ".mp3,.m4a,.wav,audio/mpeg,audio/mp3,audio/mp4,audio/x-m4a,audio/wav,audio/x-wav", className: "hidden", onChange: onChange }),
        boxContent);
}
...
function TeacherApp({ user, assigns, students, vocab, notices, dismissedNoticeIds, banner, tab, setTab, onCreate, onEdit, onDelete, onReview, onOpenNotice, onStudent, onDeleteStudent, onDismissNotice, onDismissAllNotices, onLogout, onChangePassword, onInstall, pushEnabled, onTogglePush, onSaveBanner, refresh }) {
    ...
    const studentSummary = (name) => {
        ...
        const feedback = submittableAssigns.filter(a => a.subs?.[name]?.comment).length;
    ...
    const homeworkCard = (a) => { const trackable = a.type !== 'exercise'; const submitted = activeStudents.filter(s => a.subs?.[s.name]?.submitted).length; const total = activeStudents.length; const percent = total ? Math.min(100, (submitted / total) * 100) : 0; return React.createElement("div", { key: a.id, role: "button", tabIndex: 0, onClick: () => onReview(a), onKeyDown: e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onReview(a); } }, className: "bg-white rounded-2xl border p-4 cursor-pointer active:scale-[.99]", style: { borderColor: BORDER } },
        ...
        React.createElement("div", { className: "flex-1 overflow-y-auto p-4" },
            ...
            tab === 'students' && React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "space-y-2" },
                    activeStudents.map(s => React.createElement("div", { key: s.name, className: "w-full bg-white rounded-2xl border p-3.5 flex items-center text-left gap-3", style: { borderColor: BORDER } },
            ...
            tab === 'notice' && React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "bg-white rounded-2xl border p-4", style: { borderColor: BORDER } },
                    ...
                    React.createElement("button", { disabled: savingNotice, onClick: saveDraft, className: "mt-4 w-full h-11 rounded-xl font-black disabled:opacity-50", style: { background: C, fontSize: 13 } }, savingNotice ? '저장 중...' : '\uC800\uC7A5')))),
...
function TeacherReview({ a, students, initialStudent, onBack, onHome, onSave }) {
    const [filter, setFilter] = useState('all');
    const active = students.filter(s => s.active);
    const list = active.filter(s => filter === 'all' || (filter === 'submitted' ? !!a.subs?.[s.name]?.submitted : !a.subs?.[s.name]?.submitted));
        ...
        React.createElement("div", { className: "flex-1 overflow-y-auto" },
            ...
            sel && React.createElement("section", { className: "px-5 pt-4 pb-6" }, sub?.submitted ? React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "mt-4" },
                    React.createElement("label", { className: "block mb-2 text-[14px] leading-5 font-bold text-[#4A5565]" }, "피드백 작성"),
                    ...
                    React.createElement("button", { disabled: !comment.trim(), onClick: () => onSave(sel, comment), className: "mt-3 w-full h-[56px] rounded-[16px] text-[16px] font-black text-white disabled:opacity-40", style: { background: C } }, "✓  피드백 저장"))) : React.createElement(Empty, null, "아직 제출하지 않았어요."))));
}
    ...
    return React.createElement(Frame, null,
        ...
        React.createElement("div", { className: "flex-1 overflow-y-auto p-4" },
            React.createElement("div", { className: "flex items-center gap-3 mb-5" },
                React.createElement(Avatar, { name: name, src: profile?.avatar, size: 54 }),
                React.createElement("div", { className: "flex-1" },
                    React.createElement("b", { className: "text-[18px]" }, name),
                    React.createElement("div", { className: "text-[11px] text-[#999]" }, "\uD559\uC0DD \uD604\uD669")),
                React.createElement("button", { onClick: onDelete, className: "px-3 py-1.5 rounded-lg bg-[#F1F1F1] font-bold text-[#666]", style: { fontSize: 13 } }, "\uD559\uC0DD \uC0AD\uC81C")),
            React.createElement("div", { className: "bg-white rounded-2xl border p-4", style: { borderColor: BORDER } },
            ...
            React.createElement("div", { className: "space-y-2" }, assigns.filter(a => !a.archived).map(a => React.createElement("button", { key: a.id, onClick: () => onReview(a), className: "w-full bg-white rounded-2xl border p-3.5 text-left flex gap-3 items-center", style: { borderColor: BORDER } },
                React.createElement("div", { className: "flex-1" },
...
function App() {
    ...
    const content = useMemo(() => {
        ...
        if (page === 'teacher-review' && active) {
            ...
            return active.type === 'exercise' ? React.createElement(TeacherExerciseDetail, { a: active, onBack: reviewBack, onHome: reviewHome }) : React.createElement(TeacherReview, { a: active, students: students, initialStudent: reviewStudent, onBack: reviewBack, onHome: reviewHome, onSave: feedback });
        }
