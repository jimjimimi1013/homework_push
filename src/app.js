...
function StudentApp({ user, assigns, notices, dismissedNoticeIds, vocab, banner, students, tab, setTab, onOpen, onOpenNotice, onDismissNotice, onDismissAllNotices, onLogout, onChangePassword, onAvatar, onInstall, pushEnabled, onTogglePush, refresh, onSendContact }) {
    ...
    return React.createElement(Frame, null,
        ...
        React.createElement("div", { className: "flex-1 min-h-0 overflow-y-auto px-4 py-4" },
            ...
            tab === 'contact' && React.createElement(React.Fragment, null,
                React.createElement("div", { className: "px-2 pt-2 text-center" },
                    React.createElement("h1", { className: "text-[23px] leading-8 font-black text-[#111]" }, "일하기 싫어서 만들어봤어요 🤦"),
                    React.createElement("p", { className: "mt-3 text-[14px] leading-6 text-[#555]" }, "아무 말 대잔치 금지! 아래 메시지 중 하나를 골라", React.createElement("br"), "선생님 또는 학생들에게 보내보세요 😜")),
                React.createElement("div", { className: "mt-7 overflow-hidden rounded-[20px] border bg-white px-3 shadow-[0_6px_22px_rgba(0,0,0,.035)]", style: { borderColor: '#EFEFEF' } }, CONTACT_MESSAGES.map((message, index) => {
                    const [emoji, ...words] = message.split(' ');
                    const teacherOnly = TEACHER_ONLY_CONTACT_MESSAGES.has(message);
                    return React.createElement("button", { key: message, onClick: () => chooseContact(message), className: `w-full min-h-[58px] text-left flex items-center gap-3 active:scale-[.99] ${index ? 'border-t' : ''}`, style: index ? { borderColor: '#EEEEEE' } : undefined },
                        React.createElement("span", { className: "w-11 h-11 shrink-0 rounded-2xl flex items-center justify-center text-[25px]", style: { background: teacherOnly ? '#FFF0F3' : '#FAFAFA' } }, emoji),
                        React.createElement("span", { className: "flex-1 min-w-0 text-[16px] font-black text-[#1D1D1F]" }, words.join(' ')),
                        teacherOnly && React.createElement("span", { className: "shrink-0 rounded-full px-2 py-1 text-[11px] font-bold", style: { color: C, background: '#FFF0F3' } }, "선생님에게만"),
                        React.createElement("span", { className: "shrink-0 text-[27px] font-light leading-none text-[#888]" }, "›"));
                })),
                React.createElement("div", { className: "mt-5 rounded-2xl bg-white/75 px-4 py-3 text-center text-[12px] leading-6 text-[#666]" }, "💡 결석 / 지각 메시지는 선생님에게만 전달돼요.", React.createElement("br"), "그 외 메시지는 받을 사람을 선택할 수 있어요!")),
        React.createElement(StudentNav, { tab: tab, setTab: setTab }),
        contactChoice && contactStage === 'targets' && React.createElement("div", { className: "fixed inset-0 z-[90] flex items-end bg-black/25" },
            React.createElement("button", { className: "absolute inset-0 cursor-default", onClick: closeContact, "aria-label": "대상 선택 닫기" }),
            React.createElement("div", { className: "relative w-full rounded-t-[28px] bg-white px-5 pb-[calc(24px+env(safe-area-inset-bottom))] pt-3 shadow-2xl" },
                React.createElement("div", { className: "mx-auto h-1 w-12 rounded-full bg-[#D1D1D1]" }),
                React.createElement("div", { className: "mt-5 flex items-center justify-between" }, React.createElement("h2", { className: "text-[22px] font-black text-[#111]" }, "누구에게 보낼까요?"), React.createElement("button", { onClick: closeContact, className: "text-[33px] leading-none font-light text-[#111]", "aria-label": "닫기" }, "×")),
                React.createElement("div", { className: "mt-6 space-y-2" },
                    [["👤", "선생님", "선생님에게만 보내요", '#FFF0F3', () => { setContactRecipients([]); setContactStage('confirm'); }], ["👥", "학생 선택", "특정 학생을 선택해서 보내요", '#F2F0FF', () => setContactStage('students')], ["👥", "전체 학생", "모든 학생에게 보내요", '#EEFFF0', () => { setContactRecipients(contactStudents.map(student => student.name)); setContactStage('confirm'); }]].map(([icon, title, description, color, action]) => React.createElement("button", { key: title, onClick: action, disabled: title !== '선생님' && !contactStudents.length, className: "w-full min-h-[92px] rounded-2xl border px-4 text-left flex items-center gap-4 disabled:opacity-40", style: { borderColor: '#E9E9E9' } }, React.createElement("span", { className: "w-14 h-14 rounded-2xl flex items-center justify-center text-[29px]", style: { background: color } }, icon), React.createElement("span", { className: "flex-1" }, React.createElement("b", { className: "block text-[17px] text-[#111]" }, title), React.createElement("span", { className: "mt-1 block text-[13px] text-[#555]" }, description)), React.createElement("span", { className: "text-[30px] font-light text-[#777]" }, "›"))))),
        contactChoice && contactStage === 'students' && React.createElement("div", { className: "fixed inset-0 z-[90] flex items-end bg-black/25" },
            React.createElement("button", { className: "absolute inset-0 cursor-default", onClick: closeContact, "aria-label": "학생 선택 닫기" }),
            React.createElement("div", { className: "relative w-full rounded-t-[28px] bg-white px-5 pb-[calc(20px+env(safe-area-inset-bottom))] pt-3 shadow-2xl" },
                React.createElement("div", { className: "mx-auto h-1 w-12 rounded-full bg-[#D1D1D1]" }),
                React.createElement("div", { className: "mt-5 flex items-center justify-between" }, React.createElement("button", { onClick: () => setContactStage('targets'), className: "text-[34px] leading-none font-light text-[#111]", "aria-label": "대상 선택으로 돌아가기" }, "‹"), React.createElement("h2", { className: "text-[21px] font-black text-[#111]" }, "학생 선택"), React.createElement("button", { onClick: closeContact, className: "text-[33px] leading-none font-light text-[#111]", "aria-label": "닫기" }, "×")),
                React.createElement("div", { className: "mt-5 flex items-center justify-between text-[13px] text-[#555]" }, React.createElement("span", null, "보낼 학생을 선택하세요 (1명 이상)"), React.createElement("b", { className: "text-[#222]" }, `선택 ${contactRecipients.length}명`)),
                React.createElement("div", { className: "mt-4 max-h-[43vh] overflow-y-auto" }, contactStudents.map((student, index) => { const selected = contactRecipients.includes(student.name); const colors = ['#FF87A7', '#FFC43D', '#5BC6C3', '#A99AEE', '#75A8EF']; return React.createElement("button", { key: student.name, onClick: () => toggleContactRecipient(student.name), className: "w-full h-[58px] text-left flex items-center gap-3" }, React.createElement("span", { className: "w-8 h-8 rounded-full text-white text-[13px] font-bold flex items-center justify-center", style: { background: colors[index % colors.length] } }, student.name.slice(0, 1)), React.createElement("span", { className: "flex-1 text-[16px] font-bold text-[#222]" }, student.name), React.createElement("span", { className: "w-7 h-7 rounded-full border flex items-center justify-center text-[17px] font-bold", style: { background: selected ? '#FF3E76' : '#fff', borderColor: selected ? '#FF3E76' : '#C9C9C9', color: '#fff' } }, selected ? '✓' : ''))); })),
                React.createElement("button", { disabled: !contactRecipients.length, onClick: () => setContactStage('confirm'), className: "mt-4 h-14 w-full rounded-xl text-[16px] font-black text-white disabled:opacity-50", style: { background: C } }, `보내기 (${contactRecipients.length}명)`))),
        contactChoice && contactStage === 'confirm' && React.createElement("div", { className: "fixed inset-0 z-[90] flex items-end bg-black/25" },
            React.createElement("button", { className: "absolute inset-0 cursor-default", onClick: closeContact, "aria-label": "메시지 전송 확인 닫기" }),
            React.createElement("div", { className: "relative w-full rounded-t-[28px] bg-white px-5 pb-[calc(20px+env(safe-area-inset-bottom))] pt-3 shadow-2xl" },
                React.createElement("div", { className: "mx-auto h-1 w-12 rounded-full bg-[#D1D1D1]" }),
                React.createElement("div", { className: "mt-5 flex items-center justify-between" }, React.createElement("h2", { className: "text-[22px] font-black text-[#111]" }, "메시지를 보낼까요?"), React.createElement("button", { onClick: closeContact, className: "text-[33px] leading-none font-light text-[#111]", "aria-label": "닫기" }, "×")),
                React.createElement("div", { className: "mt-5 rounded-2xl border px-4 py-5 text-[17px] font-black text-[#222]", style: { borderColor: '#FFD4DD', background: '#FFF7F8' } }, contactChoice),
                React.createElement("div", { className: "mt-4 rounded-2xl border px-4 py-4", style: { borderColor: '#E9E9E9' } }, React.createElement("b", { className: "block text-[14px] text-[#222]" }, "대상"), React.createElement("p", { className: "mt-2 text-[14px] font-bold text-[#333]" }, contactRecipientLabel)),
                React.createElement("div", { className: "mt-5 flex gap-3" },
                    React.createElement("button", { disabled: contactBusy, onClick: closeContact, className: "h-14 flex-1 rounded-xl border text-[16px] font-black text-[#222] disabled:opacity-50", style: { borderColor: '#DFDFDF' } }, "취소"),
                    React.createElement("button", { disabled: contactBusy, onClick: sendContact, className: "h-14 flex-1 rounded-xl text-[16px] font-black text-white disabled:opacity-50", style: { background: C } }, contactBusy ? "보내는 중..." : "보내기")))));
}
